...
using Microsoft.AspNetCore.Http.Extensions;  // GetDisplayUrl
  ...
  public class FileController : Controller {
    ...
    private void checkProofHeaders() {
      if (ProofKeysStore.helper != null) {
        ...
        long.TryParse(request.Headers["X-WOPI-TimeStamp"], out long timestamp100NanoSec);
        ...
        if (!String.IsNullOrEmpty(rpu_prefix)) {
          ...
          if (slashes.Length >= 4){
            var url_base = string.Join("/", slashes.Take(3)).Length + 1;
            url = rpu_prefix + url.Substring(url_base);
            Console.WriteLine("WOPI_PROOF: url behind reverse proxy: " + url);
          }
        }
        var proofSignatureBase64 = request.Headers["X-WOPI-Proof"].ToString();
        var oldProofSignatureBase64 = request.Headers["X-WOPI-ProofOld"].ToString();
